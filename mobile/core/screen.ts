import type { MaestroNode, MaestroScreen } from './types';
 ...
 * 1. A row node that carries the label as `a11y` and the value as `val`/`txt` (how iOS settings rows
 *    are shaped: `{ a11y: 'Name', txt: 'iPhone', val: 'iPhone' }`).
 ...
 *    rows, where the value sits under its label — e.g. `Device name` / `sdk_gphone64_arm64`).
 *
 * So it works across iOS and Android layouts. You never walk the tree yourself — this reads
 * `inspectScreen()` output for you.
 */
