...
/**
 * Boot an Android AVD by name and wait until it finishes booting. Launches the emulator detached
 * (so it outlives the test process — booted devices are reused, not shut down), then polls
 * `sys.boot_completed`. Throws with an actionable message if the AVD/emulator is missing or the
 * boot times out.
 */
export async function bootAndroidAvd(avdName: string, timeoutMs = 180_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    const booted = await execFileAsync(adb, ['shell', 'getprop', 'sys.boot_completed'], {
      timeout: 5_000,
    })
