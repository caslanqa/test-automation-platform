...
| scroll                                       | ⚠️ degraded — the adapter calls Maestro's bare `scroll()`, silently discarding **both** the recorded `direction` and `within`. Must either be declared unsupported or implemented as a directional swipe; silently ignoring recorded intent is exactly what this matrix exists to prevent. | ✅               | ✅                                       |
...
- **App scope is part of a locator's confidence.** Found the hard way while closing the Phase 0 gate: the
  hierarchy comes from a whole-screen capture, so it includes elements belonging to _other_ apps — the
  status bar, the notification shade, a system dialog. Tapping one during recording can succeed, and the
  engine will happily rank it 92/high, but the replay fails: Maestro scopes every command to an app id, so
  a `com.android.settings` test cannot act on a SystemUI element. **Record-time success does not imply
  replay-time success.** The engine MUST therefore treat "outside the app under test" as a confidence
  penalty with an explicit warning, using whatever the platform exposes (Android's package attribute; on
  iOS, membership in the `AppiumAUT`/application subtree). Until that lands, this is the most likely cause
  of a recorded test that passes in the inspector and fails on replay.
- **The `native` strategy must stop being a phantom.** `LocatorCandidate.strategy` includes `'native'`,
  but the engine never produces one, so the UI has a branch that can never render and adapters accept a
  `native` locator no recorder can create. Either the engine emits a real platform-native candidate
  (Android `UiSelector` / iOS predicate / XPath, ranked below `text` and above `point`) or `'native'` is
  removed from the union and kept only as a hand-authored escape hatch in `MobileLocator`. Decide in
  Phase 2; a type that lies is worse than either option.
