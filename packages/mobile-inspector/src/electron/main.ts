...
function registerIpc(window: BrowserWindow): void {
  ...
  ipcMain.handle(IpcChannels.pickSaveLocation, async (): Promise<PickPathResult | null> => {
    ...
    const defaultPath = fsSync.existsSync(testsDir) ? testsDir : PROJECT_ROOT;
    const result = await dialog.showOpenDialog(window, {
    ...
    return confineToProject(picked, window);
  });
...
/** Re-confine an absolute path picked via a native dialog to the project root, reporting and
 * returning `null` if the user somehow navigated outside it (the dialogs default inside the
 * project but don't prevent browsing elsewhere). */
