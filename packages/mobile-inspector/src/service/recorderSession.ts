...
export class RecorderSession {
  ...
  private busy = false;
  private hierarchyRefresh: Promise<void> | undefined;
  ...
  /** Enumerate existing `*.mobile.ts` files for the append picker. */
  ...
  /**
   * Create a new test or append it to an existing test file. Both paths remain confined to the
   * project; new mode refuses overwrite and append mode preserves existing content.
   */
  async save(
    mode: 'new' | 'append',
    targetPath: string,
    ...
    if (!finalRelative.endsWith('.mobile.ts')) {
      this.send({ type: 'error', message: 'target must be a .mobile.ts test file' });
      return;
    }
    const projectRoot = path.resolve(this.projectRoot);
    const target = path.resolve(projectRoot, finalRelative);
    ...
    if (mode === 'new' && exists) {
      this.send({
        type: 'error',
        message: `${finalRelative} already exists — append to it or choose a different name`,
      });
      return;
    }
...
/**
 * Append one generated test without duplicating imports or leaking its top-level `test.use()` into
 * existing tests. The appended source is scoped inside its own describe block.
 */
