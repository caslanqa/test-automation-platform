...
2. **Tap the device screen** to record a tap; drag to record a swipe. Right-click any element for ranked locator alternatives and Tap / Fill / Assert visible / Assert not visible / Wait / Copy — actions the connected driver cannot do are disabled with the reason.
3. **The editor is yours.** Type in it and the recording splices new actions into what you wrote instead of overwriting it. Undo/redo work on the timeline, not just the text.
4. **Run** executes the draft through the project's own Playwright, in the driver's project with its gate variable set, and streams the output back.
5. **Save…** writes a new file (browse the project's real directories) or appends to an existing recording, merging imports and keeping the file's existing tests.
