...
export const manifest = {
  ...
  readmeSection: [
    ...
    '```',
    '',
    '**5. Mirror your specs as cases** with `npm run tms:sync`. It prints a plan and changes nothing',
    ...
    '',
    '**7. File the real failures**, if `@pwtap/plugin-heal` is installed. `npx heal triage --json`',
    '`.heal/triage.json` classifies a red run; `npm run tms:defects` then opens a defect for every',
    '`true-fail` and mirrors the quarantine list as flaky cases. **Only `true-fail`** — a flaky test',
    'opening a defect is how a tracker stops being read.',
    '',
