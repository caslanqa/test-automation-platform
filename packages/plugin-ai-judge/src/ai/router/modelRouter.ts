...
import { type AIProvider, JudgeHttpError } from '../providers/provider.js';
import { kindForModel, providerForKind, stripPrefix } from '../providers/registry.js';
import type {
  ComplexityResult,
  JudgeInput,
  JudgeMeta,
...
/** Classify a routable model id to its provider kind (by registered prefix; default 'openai'). */
function providerOf(modelId: string): ProviderKind {
  return kindForModel(modelId);
}

/** The concrete AIProvider for a model profile. */
export function getProvider(model: ModelProfile): AIProvider {
  return providerForKind(model.provider);
}

/** Convert a routable id to the backend-native model string (strip the provider's routing prefix). */
export function toApiModel(model: ModelProfile): string {
  return stripPrefix(model.id, model.provider);
}
...
/** Synthesize a profile for a model id not present in (or not yet discovered by) the registry. */
function synthCloudProfile(modelId: string, config: AIJudgeConfig): ModelProfile {
  const lower = modelId.toLowerCase();
  return {
    id: modelId,
    provider: kindForModel(modelId),
