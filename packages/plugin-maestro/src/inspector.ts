...
class MaestroDriverSession implements DriverSession {
  ...
  async captureScreen(): Promise<ScreenFrame> {
    ...
    return {
      ...
      width: size.width,
      height: size.height,
      coordinateWidth: this.coordinateSize?.width,
      coordinateHeight: this.coordinateSize?.height,
  ...
  async connect(options: ConnectOptions): Promise<DriverSession> {
    ...
    try {
      ...
      for (let attempt = 1; ; attempt += 1) {
        ...
        try {
          // Maestro scopes every command to an app id and initializes its device connection lazily
          // on the first command. A freshly released iOS driver can briefly report "not connected",
          // so rebuild the MCP process once for that known transient startup state.
          if (options.appId) {
