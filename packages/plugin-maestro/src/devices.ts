 ...
 *
 * test.use({ mobile: devices.pixel9 });                          // from the catalog
 * test.use({ mobile: { platform: 'android', device: 'MyAvd' } }); // inline
 *
 * Add a `device` (AVD name / iOS simulator name or UDID) to have it auto-booted on first use. Create
 * one from your installed SDK/Xcode with `npm run mobile:create-device`. When a named device isn't
 * present the test SKIPS (never fails), so these examples are safe to keep.
 */
