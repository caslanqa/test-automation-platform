...
export const manifest = {
  ...
  readmeSection: [
    ...
    '',
    '- `sql` — a Knex instance for PostgreSQL, MySQL, MariaDB or SQLite. Configure with',
    '  `test.use({ db: { client: "pg", connection } })`.',
    '- `mongo` — a MongoDB `Db`. Configure with `test.use({ mongoDb: { connection, database } })`.',
    '',
