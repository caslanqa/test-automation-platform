...
export const manifest = {
  ...
  readmeSection: [
    ...
    '',
    '**1. Install the binary.** There is no npm route — k6 runs its own JavaScript runtime, not Node, and the `k6`',
    'package on npm is an autocomplete stub. Use `brew install k6` (macOS), the apt/dnf repositories (Linux),',
    '`winget install k6 --source winget` (Windows), a standalone binary, or Docker with no install at all:',
    '`docker run --rm -i -v "$PWD:/src" -w /src grafana/k6 run perf/smoke.ts`. `create-pwtap add perf` installs',
    'nothing, but it probes this machine for a package manager and prints the command that actually fits it.',
    '',
